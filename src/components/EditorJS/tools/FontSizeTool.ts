...
class FontSizeTool implements BaseTool {
    api: API;
    ...
    render() {
        const styles = getFontSizeStyles();
        document.head.insertAdjacentHTML("beforeend", `<style>${styles}</style>`);
        this.button = document.createElement('div');
        (this.button as any).type = 'div';
    ...
    toggleFontSizeMenu() {
        ...
        this.select.style.top = `${(buttonRect as any).height + 5}px`;
    ...
    surround(range: any, context: any) {
        if (!context) {
            return false;
        }
    ...
    checkState(selection: any) {
        ...
        if (parentSpan) {
            ...
            if (this.select) {
                (this.select as any).value = parentSpan.style.fontSize || '';
            }
        } else {
            this.button?.classList.remove(this.CSS.active);
            if (this.select) {
                (this.select as any).value = '';
            }
        }
    }
}

function getFontSizeStyles() {
    return `
.cdx-font-size-button { background:transparent; border:none; cursor:pointer; font-size:16px; display:flex; padding:6px; align-items:center; color:#333; }
.cdx-font-size-button:hover { border-radius:5px 0 0 5px; background:#cbcbcb7d; }
.dropdown-container { position:relative; display:inline-block; }
.dropdown-button { cursor:pointer; background-color:#f0f0f0; border:1px solid #ccc; }
.dropdown-menu { display:none; position:absolute; background-color:#fff; border:1px solid #ddd; border-radius:3px; min-width:150px; z-index:1000; box-shadow:2px 2px 15px rgba(0,0,0,0.15); }
.dropdown-menu div { padding:10px; cursor:pointer; }
.dropdown-menu div:hover { background-color:#ddd; }
`;
}
